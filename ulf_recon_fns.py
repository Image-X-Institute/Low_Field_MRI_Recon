# Recon Functions for ULF MRI Experiments - Updated by D Waddington 2022
import numpy as np
...
def poissonDiscSigpy(imSize,accel,in_seed,calib_size=10):
    if accel == 1:
      ...
      #accel = accel+0.05 # increases the acceleration factor slightly due to calibration region. This value should not be hardcoded...
...
def find_lamda_mask(ksp, GT, mps=float('nan'), calib_size=10, show_plot=True, iter = 30):
    ...
    #lamda_vals = np.array([1E-6, 1E-5, 2E-5, 5E-5, 1E-4, 2e-4, 5e-4, 1E-3, 2E-3, 5E-3, 1E-2, 2E-2, 5E-2, 1E-1])
    #lamda_vals = np.array([1E-5, 3E-5, 1E-4, 3E-4, 6E-4, 1E-3, 3E-3, 6E-3, 1E-2, 3E-2, 6E-2, 1E-1, 3E-1, 1])
    #lamda_vals = np.array([1E-6, 1E-5, 1E-4, 1E-3, 2E-3, 5E-3, 1E-2, 2E-2, 5E-2, 1E-1])
    ...
    #for j in range(no_masks):
        #masks[:,:,j] = poissonDiscSigpy(GT.shape,R, j, calib_size)
        #print('Mask No. ',j)
